...
import { exportToExcel, downloadExcel, formatImportsForExcel, IMPORT_COLUMNS } from '@/lib/excel'
...
export default function ImportsPage() {
    ...
    useEffect(() => {
        Promise.all([
            ...
            fetch('/api/imports?limit=100').then(r => r.json()),
        ]).then(([productsData, warehousesData, importsData]) => {
    ...
    const filteredImports = useMemo(() => {
        ...
        return result
    }, [imports, filterProduct, filterWarehouse, search, sortField, sortDirection])

    const totalAmount = filteredImports.reduce((sum, item) => sum + (item.total * item.price), 0)
    ...
    const handleExportExcel = () => {
        ...
        const formattedData = formatImportsForExcel(filteredImports)
        const blob = exportToExcel(formattedData, IMPORT_COLUMNS, 'nhap-kho')
        downloadExcel(blob, `nhap-kho-${new Date().toISOString().split('T')[0]}`)
        toast({ title: 'Thành công', description: `Đã xuất ${filteredImports.length} bản ghi` })
    }
    ...
    const handleSave = async () => {
        ...
        setSaving(true)
        try {
            // Save each item as a separate import record
            ...
            setIsCreating(false)
            setItems([])
            setFormData({ warehouseId: '', date: new Date().toISOString().split('T')[0], note: '', isPaid: false })
            toast({ title: 'Thành công', description: 'Đã tạo nhập kho' })
            ...
            toast({ title: 'Lỗi', description: 'Không thể tạo nhập kho', variant: 'destructive' })
        } finally {
            setSaving(false)
        }
    }
        ...
        return (
            <div className="space-y-6 animate-fade-in">
                <div className="flex items-center justify-between">
                    <h2 className="text-2xl font-bold">Tạo nhập kho</h2>
                    <Button variant="outline" onClick={() => setIsCreating(false)}>Hủy</Button>
                </div>

                <Card className="border-border/50">
                    <CardHeader><CardTitle>Thông tin nhập kho</CardTitle></CardHeader>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div className="flex items-end pb-2">
                                <div className="flex items-center space-x-2">
                                    <Checkbox id="isPaid" checked={formData.isPaid} onCheckedChange={(checked) => setFormData({ ...formData, isPaid: checked as boolean })} />
                                    <Label htmlFor="isPaid" className="cursor-pointer">Đã thanh toán</Label>
                                </div>
                            </div>
                        </div>
                        <div className="space-y-2">
                    ...
                    <CardContent className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                            <div className="space-y-2">
                                ...
                                <Select value={currentItem.productId} onValueChange={(v) => setCurrentItem({ ...currentItem, productId: v })}>
            ...
            <Tabs value={viewMode} onValueChange={(v) => setViewMode(v as 'products' | 'entries')}>
                <div className="flex items-center justify-between gap-4 flex-wrap mb-4">
                    <TabsList>
                        ...
                        <TabsTrigger value="entries">Theo phiếu nhập</TabsTrigger>
                    </TabsList>
                    <div className="flex gap-2">
                        <Button variant="outline" onClick={handleExportExcel}><Download className="h-4 w-4 mr-2" />Xuất Excel</Button>
                        <Button onClick={() => setIsCreating(true)}><Plus className="h-4 w-4 mr-2" />Tạo nhập kho</Button>
                    </div>
                </div>
                ...
                <TabsContent value="products" className="mt-0">
                    <Card className="border-border/50">
                        <CardHeader className="pb-3"><CardTitle>Nhập kho ({filteredImports.length} sản phẩm)</CardTitle></CardHeader>
                        <CardContent className="p-0">
                            <Table>
                                <TableHeader>
                                    <TableRow>
                                        ...
                                        <TableHead className="cursor-pointer" onClick={() => handleSort('date')}>
                                            <div className="flex items-center gap-1">Ngày nhập <ArrowUpDown className="h-3 w-3" /></div>
                                        </TableHead>
                                    ...
                                    {filteredImports.length === 0 ? (
                                        ...
                                        filteredImports.map((item) => (
                                            <TableRow key={item.id}>
                                                ...
                                                <TableCell className="text-right">{formatCurrency(item.total * item.price)}</TableCell>
                                                <TableCell>{formatDate(item.reportDate)}</TableCell>
                                                <TableCell>
                                                    <Badge variant={item.isPaid ? 'default' : 'secondary'}>
                                                        {item.isPaid ? 'Đã TT' : 'Chưa TT'}
                                                    </Badge>
                                                </TableCell>
                                            </TableRow>
                                        ))
                                ...
                                <TableFooter>
                                    <TableRow>
                                        <TableCell colSpan={5} className="text-right font-bold">Tổng tiền nhập:</TableCell>
                ...
                <TabsContent value="entries" className="mt-0">
                    <Card className="border-border/50">
                        <CardHeader className="pb-3"><CardTitle>Danh sách phiếu nhập ({filteredImports.length})</CardTitle></CardHeader>
                        <CardContent className="p-0">
                            <Table>
                                <TableHeader>
                                    <TableRow>
                                    ...
                                    {filteredImports.length === 0 ? (
                                        ...
                                        filteredImports.map((entry) => (
                                            <TableRow key={entry.id}>
                                                ...
                                                <TableCell className="text-right">{formatCurrency(entry.total * entry.price)}</TableCell>
                                                <TableCell>{formatDate(entry.reportDate)}</TableCell>
                                                <TableCell>
                                                    <Badge variant={entry.isPaid ? 'default' : 'secondary'}>
                                                        {entry.isPaid ? 'Đã TT' : 'Chưa TT'}
                                                    </Badge>
                                                </TableCell>
                                ...
                                <TableFooter>
                                    <TableRow>
                                        <TableCell colSpan={3} className="text-right font-bold">Tổng tiền nhập:</TableCell>
                                        <TableCell className="text-right font-bold text-primary">{formatCurrency(totalAmount)}</TableCell>
